...
public class TestGenericServices extends GeoTest {
    ...
    public void testMunicipalityService() throws IOException {
        ...
        Assert.assertEquals("33960e68-2f0a-4cb0-bb3d-02d9f0b21304", munipialicity1.get("uuid").textValue());
        Assert.assertEquals("5cc15446cddb4633b6832847b6f5d66d", munipialicity1.get("sumiffiik").textValue());
        Assert.assertEquals(956, munipialicity1.get("kommunekode").asInt());
    ...
    public void testPostcodeService() throws IOException {
        ResponseEntity<String> response = this.lookup("/geo/postcode/1/rest/search?fmt=dataonly&kode=3910");
        Assert.assertEquals(200, response.getStatusCode().value());
        ObjectNode responseContent = (ObjectNode) objectMapper.readTree(response.getBody());
        ObjectNode munipialicity1 = (ObjectNode) responseContent.get("results").get(0);
    ...
    public void testLocalityService() throws IOException {
        ...
        ObjectNode responseContent = (ObjectNode) objectMapper.readTree(response.getBody());
        ObjectNode munipialicity1 = (ObjectNode) responseContent.get("results").get(0);
        Assert.assertEquals("f0966470-f09f-474d-a820-e8a46ed6fcc7", munipialicity1.get("uuid").textValue());
        Assert.assertEquals("4F46D110-E6AD-46D1-B403-B12063152564", munipialicity1.get("sumiffiik").textValue());
        Assert.assertEquals("0600", munipialicity1.get("lokalitetskode").textValue());
        Assert.assertNull(munipialicity1.get("registreringFra"));
