...
SELECT *
  FROM [Datafordeler_Test].[dbo].[cvr_record_company] company
	JOIN [Datafordeler_Test].[dbo].[cvr_record_company_unit_relation] unit ON (unit.companyRecord_id = company.id)
  WHERE cvrNumber = ########

-- Find companydetails
SELECT *-- DISTINCT company.cvrNumber, name1.name
  FROM [Datafordeler_Test].[dbo].[cvr_record_company] company
	JOIN [Datafordeler_Test].[dbo].[cvr_record_address] addressItem ON (addressItem.companyRecord_id = company.id)
	...
	WHERE company.cvrNumber='########' AND addressItem.validTo IS NULL AND name1.validTo IS NULL AND add_municipality.validTo IS NULL AND metadata.validTo IS NULL

-- Find cvr-numrene for aktive p-numre uden cvr-numre
SELECT DISTINCT newestCvrRelation FROM [Datafordeler_Test].[dbo].[cvr_record_unit_metadata]
...
INSERT INTO [Datafordeler_Test].[dbo].[cvr_config]  (
id, companyRegisterDirectLookupAddress, companyRegisterDirectLookupCertificate, companyRegisterDirectLookupPassword,
companyRegisterDirectLookupPasswordEncrypted, companyRegisterPassword, companyRegisterPasswordEncrypted, companyRegisterQuery, companyRegisterScrollAddress,
companyRegisterStartAddress, companyRegisterType, companyRegisterUsername, companyUnitRegisterPassword, companyUnitRegisterPasswordEncrypted, companyUnitRegisterQuery, companyUnitRegisterScrollAddress, companyUnitRegisterStartAddress, companyUnitRegisterType, companyUnitRegisterUsername, participantRegisterDirectLookupAddress, participantRegisterDirectLookupCertificate, participantRegisterDirectLookupPassword, participantRegisterDirectLookupPasswordEncrypted, participantRegisterPassword, participantRegisterPasswordEncrypted, participantRegisterQuery, participantRegisterScrollAddress, participantRegisterStartAddress, participantRegisterType, participantRegisterUsername, pullCronSchedule
)
SELECT
id, companyRegisterDirectLookupAddress, companyRegisterDirectLookupCertificate, companyRegisterDirectLookupPassword,
companyRegisterDirectLookupPasswordEncrypted, companyRegisterPassword, companyRegisterPasswordEncrypted, companyRegisterQuery, companyRegisterScrollAddress,
