...
public class PullTest extends GapiTestBase {
    ...
    public void schedule() throws SchedulerException, InterruptedException {
        ...
        Assertions.assertEquals(2, taskListener.size(TaskListener.Event.jobToBeExecuted));
        Assertions.assertEquals(2, taskListener.size(TaskListener.Event.jobWasExecuted));

        Thread.sleep(2000);
        // Three seconds have passed, should now have executed exactly four times (initial plus 3 seconds)
        Assertions.assertEquals(4, taskListener.size(TaskListener.Event.jobToBeExecuted));
        Assertions.assertEquals(4, taskListener.size(TaskListener.Event.jobWasExecuted));
        ...
        Thread.sleep(4000);
        Assertions.assertEquals(evenSecond ? 3 : 2, taskListener.size(TaskListener.Event.jobToBeExecuted));
        Assertions.assertEquals(evenSecond ? 3 : 2, taskListener.size(TaskListener.Event.jobWasExecuted));
        ...
        Assertions.assertEquals(0, taskListener.size(TaskListener.Event.jobToBeExecuted));
        Assertions.assertEquals(0, taskListener.size(TaskListener.Event.jobWasExecuted));
    }
